...
var BitmapFont = class {
  ...
  width(text, spacing = 0) {
    return text.length * (this.meta.cell + spacing) - spacing;
  }
  draw(ctx, text, x, y, variant = "white", scale = 1, spacing = 0) {
  ...
  /** centred text */
  drawCentered(ctx, text, cx, y, variant = "white", scale = 1, spacing = 0) {
...
var Assets = class {
  ...
  async load(onProgress) {
    ...
    const imgs = await Promise.all(names.map(async (n) => {
      const im = await loadImage(ASSET_ROOT + files[n]);
      tick();
      return im;
    }));
    this.sheets = Object.fromEntries(names.map((n, i) => [n, imgs[i]]));
    const [fontImg, fontMeta] = await Promise.all([
      loadImage(ASSET_ROOT + "sprites/font.png"),
      fetch(ASSET_ROOT + "sprites/font.json").then((r) => r.json())
    ]);
    tick();
    tick();
...
var WAND_FREEZE = 1.5;
var WAND_BONUS = 10;
var TIMEZONE_PERIOD = 1.7;
var TIMEZONE_BONUS = 3;
var DEATH_PENALTY = 1e3;
var FINISH_BONUS = 2e4;
...
function screenDirToWorld(ax, ay) {
...
var Renderer = class {
  ...
  drawSprites(sprites) {
    ...
    for (const s of sprites) {
      ...
      if (s.alpha !== void 0 && s.alpha < 1) this.ctx.globalAlpha = s.alpha;
  ...
  drawHud(scoreText, timeText, opts) {
    const ctx = this.ctx;
...
var Sound = class {
  ...
  rollGain = null;
  constructor() {
    const mv = localStorage.getItem("mm_music");
    if (mv) this.musicVolume = +mv;
    const sv = localStorage.getItem("mm_sfx");
    if (sv) this.sfxVolume = +sv;
...
var HeightMap = class {
  ...
  zOf(c, u, v) {
    if (c.pieces) {
      ...
      for (const p2 of c.pieces) {
        const z = (p2.a + p2.b * D8 + p2.c * S4) / (1 + p2.c);
        if (S4 - z <= p2.yMax) return z;
      }
...
function highestBelow(level, u, v, zRef, wallMax = WALL_MAX, ignore) {
  let best = null;
...
function topAt(level, u, v) {
  return supportAt(level, u, v, 1e9, 0);
}
function zonesAt(level, u, v, z) {
  const out = [];
  for (const zn of level.zones) {
    if (!inRect(zn, u, v)) continue;
    if (zn.zMin !== void 0 && z < zn.zMin) continue;
    if (zn.zMax !== void 0 && z > zn.zMax) continue;
    out.push(zn);
  }
  return out;
}
function pipeAt(level, u, v, z) {
  for (const p of level.pipes) {
    if (!inRect(p, u, v)) continue;
    if (p.zMin !== void 0 && z < p.zMin) continue;
    if (p.zMax !== void 0 && z > p.zMax) continue;
    return p;
  }
  return null;
}
var LevelBuilder = class {
   ...
   */
  ramp(x, y, lenU, lenV, z, gu, gv, name) {
    const { u, v } = this.uv(x, y, z);
    return this.rect(u, v, u + lenU, v + lenV, z, gu, gv, name);
  }
  rect(u0, v0, u1, v1, z0, gu = 0, gv = 0, name, kind = "floor") {
    const s = { id: this.nextId++, u0, v0, u1, v1, z0, gu, gv, kind, name };
   ...
   */
  band(x0, x1, yTop, lenS, zTop, gS = 0, gD = 0, name) {
    const s0 = Math.round((yTop + zTop) / HALF_H * 2) / 2;
    const d0 = Math.round(x0 / HALF_W * 2) / 2, d1 = Math.round(x1 / HALF_W * 2) / 2;
    const s = { id: this.nextId++, u0: s0, v0: d0, u1: s0 + lenS, v1: d1, z0: zTop, gu: gS, gv: gD, kind: "floor", sd: true, name };
    this.def.surfaces.push(s);
    return s;
  }
  /**
  ...
  wallBand(x0, x1, y0, y1, zRef, name) {
    ...
    const s = { id: this.nextId++, u0: s0, v0: d0, u1: s1, v1: d1, z0: 0, gu: 0, gv: 0, kind: "wall", sd: true, name };
    this.def.surfaces.push(s);
    return s;
  }
  /** Solid obstacle from a world-space rectangle around a map pixel at height z. */
  wallAt(x, y, z, halfU, halfV, name) {
    const { u, v } = this.uv(x, y, z);
  ...
  bandBelow(from, lenS, gS = 0, gD = 0, name, d0, d1) {
    const S0 = from.u1;
    const D0 = d0 ?? from.v0;
    const z0 = from.z0 + from.gu * (S0 - from.u0) + from.gv * (D0 - from.v0);
    const s = { id: this.nextId++, u0: S0, v0: D0, u1: S0 + lenS, v1: d1 ?? from.v1, z0, gu: gS, gv: gD, kind: "floor", sd: true, name };
    this.def.surfaces.push(s);
    return s;
  }
  /** Rectangle continuing from an existing surface's edge (same height at the shared edge). */
  extend(from, side, lenU, lenV, gu = 0, gv = 0, name) {
    let u0, v0;
    if (side === "u1") {
      u0 = from.u1;
      v0 = from.v0;
  ...
  pipe(p) {
    this.def.pipes.push(p);
  }
  hazard(h) {
    this.def.hazards.push(h);
  }
  build() {
...
function attachHeightMap(def, hm) {
  def.heightmap = hm;
  const hmSurfaces = hm.comps.map((comp) => ({
    id: 1e3 + comp.id,
    u0: 0,
    v0: 0,
    u1: 0,
...
function surfaceCorners(s) {
  if (s.sd) {
    const c = (S, D) => ({ u: (S + D) / 2, v: (S - D) / 2 });
...
var Marble = class {
  ...
  substep(level, input, h, events) {
    ...
    const sp = this.speed;
    if (sp > MAX_SPEED) {
      this.vu *= MAX_SPEED / sp;
      this.vv *= MAX_SPEED / sp;
    }
  ...
  tryMove(level, nu, nv, zRef, events) {
    ...
    if (!blocked(nu, this.v)) {
      this.u = nu;
      this.vv = -this.vv * BOUNCE;
      this.bounceEvt(sp, events);
      return true;
    }
    if (!blocked(this.u, nv)) {
      this.v = nv;
      this.vu = -this.vu * BOUNCE;
      this.bounceEvt(sp, events);
      return true;
    }
    this.vu = -this.vu * BOUNCE;
  ...
  collideBall(ou, ov, oz, ovu, ovv, otherMass, radius) {
    ...
    this.u += nx * overlap * 0.6;
    this.v += ny * overlap * 0.6;
...
// src/levels/stage1.ts
var L = new LevelBuilder({
  id: 1,
  name: "PRACTICE RACE",
  music: "practice",
  image: "stages/stage1.png",
  width: 290,
  height: 577,
  timeAdd: 60,
  carryTime: false
...
var Z_TOP = 100;
var hills = L.band(0, 290, -20, 50, 140, -0.36, 0, "hills");
...
L.bandRails(chuteB);
L.strip(205, 448, 44, 40, 540, 38, 4.6, "goalCorridor", 0.5);
...
var cp1 = L.uv(135, 300, Z_TOP);
L.checkpoint(cp1.u, cp1.v);
var cp2 = L.uv(172, 402, 70);
L.checkpoint(cp2.u, cp2.v);
var cp3 = L.uv(196, 460, 40);
L.checkpoint(cp3.u, cp3.v);
var z1 = L.uv(135, 300, Z_TOP);
L.zone("checkpoint", z1.u - 6, z1.v - 6, z1.u + 6, z1.v + 6, 1, "cp1", Z_TOP - 8, Z_TOP + 8);
var z2 = L.uv(172, 402, 70);
L.zone("checkpoint", z2.u - 3, z2.v - 3, z2.u + 3, z2.v + 3, 2, "cp2", 55, 85);
var z3 = L.uv(196, 460, 40);
L.zone("checkpoint", z3.u - 3, z3.v - 3, z3.u + 3, z3.v + 3, 3, "cp3", 28, 55);
for (const [px, py, pz] of [[236, 396, 61], [30, 441, 57], [232, 486, 61]]) {
  const c = L.uv(px, py, pz);
  L.rect(c.u - 2.2, c.v - 2.2, c.u + 2.2, c.v + 2.2, pz, 0, 0, "pad");
}
var padA = L.uv(236, 395, 61);
L.zone("bonus", padA.u - 1.6, padA.v - 1.6, padA.u + 1.6, padA.v + 1.6, 5e3, "padA", 50, 75);
var padB = L.uv(30, 440, 57);
L.zone("bonus", padB.u - 1.6, padB.v - 1.6, padB.u + 1.6, padB.v + 1.6, 1e3, "padB", 45, 70);
var padC = L.uv(232, 485, 61);
L.zone("bonus", padC.u - 1.6, padC.v - 1.6, padC.u + 1.6, padC.v + 1.6, 1e3, "padC", 50, 75);
var goal = L.uv(40, 538, 38);
L.zone("bonus", goal.u - 3, goal.v - 3, goal.u + 3, goal.v + 3, 1e3, "goalflags", 25, 55);
...
for (const [x, y, du, dv] of [[27, 515, 3, -1], [91, 567, 3, -1], [165, 553, 3, 1]]) {
  const p = L4.uv(x, y, 100);
  L4.hazard({ kind: "risers", u: p.u, v: p.v, z: p.z, size: [3, 3], period: 3.2, phase: Math.random() * 3, launch: { du, dv } });
}
{
  const p = L4.uv(130, 632, 100);
  L4.hazard({ kind: "catapult", u: p.u, v: p.v, z: p.z, launch: { du: 14, dv: 6 } });
}
...
var Steelie = class extends Hazard {
  ball = new Marble();
  wanderT = 0;
  wu = 0;
  wv = 0;
  ...
  update(dt, ctx) {
    ...
    if (target.phase === "alive" && d < 14 && Math.abs(target.z - b.z) < 30) {
      ...
      this.wanderT -= dt;
      if (this.wanderT <= 0) {
        this.wanderT = 0.8 + ctx.rng() * 1.2;
        const a = ctx.rng() * Math.PI * 2;
        this.wu = Math.cos(a) * 0.4;
        this.wv = Math.sin(a) * 0.4;
      }
      ax = this.wu;
      ay = this.wv;
    }
...
var Worm = class extends Hazard {
  ...
  update(dt, ctx) {
    ...
    let tu, tv;
    if (target.phase === "alive" && d < 9 && Math.abs(target.z - this.z) < 12) {
      tu = target.u;
      tv = target.v;
    } else {
      tu = this.spawn.u + Math.sin(this.t * 0.5) * range * 0.6;
      tv = this.spawn.v + Math.cos(this.t * 0.37) * range * 0.6;
    }
    const dx = tu - this.u, dy = tv - this.v;
    const m = Math.hypot(dx, dy) || 1;
    if (m > 0.3) {
      this.dirU = dx / m;
      this.dirV = dy / m;
    ...
    if (sup && Math.abs(sup.z - this.z) < 6 && dist({ u: nu, v: nv }, this.spawn) < range + 1) {
      this.u = nu;
      this.v = nv;
      this.z = sup.z;
    } else {
    ...
    for (const mb of ctx.marbles) {
      ...
      if (dd < 1 && Math.abs(mb.z - this.z) < 10) {
...
var Slime = class extends Hazard {
  ...
  update(dt, ctx) {
    ...
    if (sup && Math.abs(sup.z - this.z) < 6) {
      this.u = tu;
      this.v = tv;
      this.z = sup.z;
    }
...
var Vacuum = class extends Hazard {
  ...
  drawOverlay(ctx2d, project, time) {
    ...
    for (let i = 0; i < 8; i++) {
      const phase = (time * 4 + i * 0.42) % 1;
      const r = (1 - phase) * 26;
      const angle = i * 0.785 + phase * 2;
  ...
  update(dt, ctx) {
    ...
    for (const b of this.units) {
      ...
      if (target.phase === "alive" && !target.inPipe && Math.abs(b.x - sx) < 7 && Math.abs(b.y - sy) < 9 && ctx.rng() < 0.35) {
        this.zapT = 0.5;
        this.zapAt = { u: target.u, v: target.v, z: target.z };
...
var Wand = class extends Hazard {
  next = 0;
  active = 0;
  target = null;
  granted = false;
  reset(ctx) {
    this.next = 6 + ctx.rng() * 10;
    this.active = 0;
    this.target = null;
  }
  update(dt, ctx) {
    if (this.active > 0 && this.target) {
      ...
      if (this.active <= 0) {
        ...
        if (!this.granted) {
          this.granted = true;
          ctx.onEvent({ type: "wand", marble: this.target });
        }
        this.target = null;
...
var Catapult = class extends Hazard {
  t = 0;
  flipped = 0;
  cooldown = 0;
  reset(ctx) {
  ...
  update(dt, ctx) {
    ...
    if (this.cooldown <= 0) {
      ...
      for (const mb of ctx.marbles) {
        if (mb.phase !== "alive" || mb.inPipe) continue;
        const d = dist(this, mb);
        if (d < 1 && Math.abs(mb.z - this.z) < 10) {
          this.cooldown = 1.4;
          this.flipped = 1;
          mb.u = this.u;
          mb.v = this.v;
...
// src/game/screens.ts
var LETTERS = ["ABCDEFG", "HIJKLMN", "OPQRSTU", "VWXYZ"];
var NAME_MAX = 6;
var Screens = class {
  constructor(g) {
    this.g = g;
...
3. Race as the Red AI Marble against my Blue Marble and beat me to the finish line!`;
  ...
  update(dt) {
    ...
    switch (g.screen) {
      ...
      case "control": {
        for (const p of presses) {
          if (p === "ArrowUp" || p === "ArrowDown" || p === "KeyW" || p === "KeyS") {
            this.cursor = 1 - this.cursor;
            g.sound.sfx("tick", 0.4);
          }
      ...
      case "congrats": {
        ...
        if (this.idle > 2.5 && tally.drained >= tally.total && !this.g.scoreSubmitted) {
          void g.submitScore();
        }
  ...
  updateName(presses, clicks) {
    ...
    for (const p of presses) {
      if (p.startsWith("Key") && p.length === 4) {
        const ch = p[3];
        if (g.playerName.length < NAME_MAX) {
          g.playerName += ch;
          g.sound.sfx("tick", 0.4);
        }
        for (let r = 0; r < LETTERS.length; r++) {
          const c = LETTERS[r].indexOf(ch);
          if (c >= 0) this.nameCur = { r, c };
        }
        continue;
      }
      if (p === "Backspace") {
  ...
  renderCongrats() {
    ...
    const rows = [
      ...
      [`${secLeft} SEC LEFT X 1000`, fmtScore(secLeft * 1e3)],
  ...
  showConnect(show) {
    ...
    if (show) {
      ...
      const ta = document.getElementById("connect-text");
      if (ta) ta.value = text;
      const lob = document.getElementById("connect-lobby");
      if (lob) lob.textContent = g.lobbyId;
      ...
      if (btn) {
        btn.onclick = async () => {
          ...
          setTimeout(() => {
            btn.textContent = "Copy / Add to Clipboard";
          }, 1500);
        };
      }
    }
...
// src/render/ai_tracker.ts
var AITrackerOverlay = class {
  smoothX = 0;
  smoothY = 0;
  initialized = false;
  lastTime = 0;
   ...
   */
  rotoscopeLine(ctx, x0, y0, x1, y1, color, inkColor = "#05070d", seedOffset = 0) {
   ...
   */
  rotoscopePoly(ctx, pts, fillColor, strokeColor, seedOffset = 0) {
   ...
   */
  render(ctx, target, viewW, viewH, time) {
    ...
    const dt = Math.min(0.05, Math.max(1e-3, time - this.lastTime));
    this.lastTime = time;
...
var Game = class {
  ...
  carried = 0;
  introPool = 0;
  // seconds still to transfer during the intro
  deaths = 0;
  popups = [];
  progressMax = -Infinity;
  bonusTaken = /* @__PURE__ */ new Set();
  checkpointIdx = 0;
  respawnT = 0;
  timezoneT = 0;
  timezoneTag = 0;
  ...
  updateIntro(dt) {
    ...
    if (this.introPool > 0) {
      if (this.t > 0.6 && this.stage.music && !this.musicStarted) {
        ...
        this.sound.playBgm(this.stage.music);
      }
      const rate = 22 * dt;
      ...
      if (this.introPool <= 1e-3) {
        this.introPool = 0;
        this.introDone = this.t;
      }
  ...
  hazardCtx(dt) {
    ...
    return {
      level: this.stage,
      assets: this.assets,
      marbles: [this.marble, ...this.others.filter((m) => m.phase === "alive")],
      local: this.marble,
      time: this.raceTime,
      rng: this.rng,
      onEvent: (e) => this.onHazardEvent(e)
    };
  }
  onHazardEvent(e) {
    switch (e.type) {
      case "sfx":
        this.sound.sfx(e.name, e.vol ?? 1);
        break;
      case "wand":
  ...
  checkZones() {
    ...
    while (prog >= this.progressMax + PROGRESS_STEP) {
      this.progressMax += PROGRESS_STEP;
      this.score += PROGRESS_POINTS;
    }
    const pipe = pipeAt(s, m.u, m.v, m.z);
    if (pipe && m.grounded) {
      m.inPipe = true;
      m.pipeT = pipe.duration;
      m.pipeExit = pipe.exit;
      this.sound.sfx("springboard", 0.7);
      if (pipe.bonus) {
        const key = `pipe:${pipe.exit.u},${pipe.exit.v}`;
        if (!this.bonusTaken.has(key)) {
          this.bonusTaken.add(key);
          this.pendingPipeBonus = pipe.bonus;
        }
      }
      return;
    }
    if (this.pendingPipeBonus && !m.inPipe) {
      this.award(this.pendingPipeBonus, m);
      this.pendingPipeBonus = 0;
    }
    ...
    for (const z of zonesAt(s, m.u, m.v, m.z)) {
      switch (z.kind) {
        case "bonus": {
          ...
          if (!this.bonusTaken.has(key)) {
            this.bonusTaken.add(key);
            this.award(z.value ?? 1e3, m);
          }
          break;
        }
        case "checkpoint": {
          const idx = z.value ?? 0;
          if (idx > this.checkpointIdx) {
  ...
  updateTimeBonus(dt) {
    ...
    if (this.t > 1 && this.bonusCount < this.bonusTotal) {
      ...
      this.score += step;
      this.tickAcc += dt;
      if (this.tickAcc > 0.08) {
        this.tickAcc = 0;
        this.sound.sfx("tick", 0.3, 1.2);
      }
    }
  ...
  respawn() {
    ...
    if (this.pendingRespawnAtStart) {
      cp = s.start;
      this.checkpointIdx = 0;
      this.pendingRespawnAtStart = false;
    }
  ...
  centerCameraOnMarble(snap, dt = 0) {
...
async function loadHeightMap(stage) {
